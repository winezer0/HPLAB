...
from libs.lib_filter_srting.filter_base import has_chinese_func, analyse_string_per_char, format_rule_list


# 过滤（账号,密码）元组列表
def filter_pair_tuples_by_length(tuple_list, min_len_name, max_len_name, min_len_pass, max_len_pass,
                                 ignore_empty, ignore_chinese, ignore_symbols):
    # 对(账号,密码)元组列表进行长度过滤
    ...
    tuple_list = [(name_, pass_) for name_, pass_ in tuple_list
                  if ((ignore_empty and name_ == "")
                      or (ignore_chinese and has_chinese_func(name_))
                  ...
                  and ((ignore_empty and pass_ == "")
                       or (ignore_chinese and has_chinese_func(pass_))
...
# 对字符串列表进行长度过滤
def filter_string_list_by_length(string_list, min_len_str, max_len_str, ignore_empty, ignore_chinese, ignore_symbols):
    # 对字符串列表进行长度过滤
    ...
    string_list = [string for string in string_list
                   ...
                   or (ignore_chinese and has_chinese_func(string))
...
# 排除包含指定符号的字符串
def filter_string_list_by_symbols(string_list, ban_symbols_str, ignore_empty, ignore_chinese, ignore_symbols):
    string_list = [string for string in string_list
                   if (ignore_empty and string == "")
                   or (ignore_chinese and has_chinese_func(string))
                   or (ignore_symbols and list_ele_in_str(ignore_symbols, string, default=False))
...
# 排除包含指定符号的字符串
def filter_tuple_list_by_symbols(tuple_list, ban_symbols_name, ban_symbols_pass,
                                 ignore_empty, ignore_chinese, ignore_symbols):
    tuple_list = [(name_, pass_) for name_, pass_ in tuple_list
                  if ((ignore_empty and name_ == "")
                      or (ignore_chinese and has_chinese_func(name_))
...
# 基于输入的规则列表判断字符是否被提取
def extract_string_list_by_char_type(string_list, expected_rules_str, ignore_empty, ignore_chinese, ignore_symbols):
    # 基于输入的规则列表判断字符是否被提取
...
# 基于输入的规则列表判断字符是否被提取
def extract_tuple_list_by_char_type(tuple_list, expected_rules_name, expected_rules_pass,
                                    ignore_empty, ignore_chinese, ignore_symbols):
    # 基于输入的规则列表判断字符是否被提取
...
# 基于输入的规则列表判断字符是否被排除
def exclude_string_list_by_char_type(string_list, expected_rules_str, ignore_empty, ignore_chinese, ignore_symbols):
    # 基于输入的规则列表判断字符是否被排除
    expected_rules_str = format_rule_list(expected_rules_str)
    # has_digit, has_upper, has_lower, has_symbol
    string_list = [string for string in string_list
...
# 基于输入的规则列表判断字符是否被排除
def exclude_tuple_list_by_char_type(tuple_list, expected_rules_name, expected_rules_pass,
                                    ignore_empty, ignore_chinese, ignore_symbols):
    expected_rules_name = format_rule_list(expected_rules_name)
    expected_rules_pass = format_rule_list(expected_rules_pass)
