...
def gen_key_list(keyboard_x2d, len_list=[]):
    ...
    for length in len_list:
        tmp_list = ["".join(lst[:length]) for lst in keyboard_x2d]
        line_result.extend(tmp_list)
...
if __name__ == '__main__':
    ...
    for file_path, options in base_dict.items():
        ...
        # # 仅已指定字符开头的元素
        # if options["starts"]:
        #     data_list = [string for string in data_list if any(string.startswith(start) for start in options["starts"])]
        # 同时包含 字母、数字、符号中的任意两项
